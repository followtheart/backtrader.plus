 ...
 * @brief 线程池框�?- 用于并行策略优化
 * 
 * 对应 Python backtrader �?multiprocessing.Pool
 * 提供任务并行执行能力，支持策略参数优化�?
 * 
 * 特�?
 * - 固定大小线程�?
 ...
 * @brief 线程池类
 * 
 * 使用固定数量的工作线程处理任务队列中的任�?
 */
class ThreadPool {
public:
    /**
     * @brief 构造函�?
     * @param numThreads 工作线程数量，默认为硬件线程�?
     */
...
struct OptimizationProgress {
    Size completed = 0;        // 已完成数�?
    Size total = 0;            // 总数�?
    double percentage = 0;     // 完成百分�?
