/**
 * @file vectorized.hpp
 * @brief 向量化计算框�?- 对应 Python �?runonce 模式
 * 
 * 实现 once/preonce/oncestart 向量化计算模�?
 * 提供批量处理接口以提高性能�?
 * 
 * Python lineiterator.py 中的对应方法:
 * - _once(): 主入口，设置缓冲区大小并调用子方�?
 * - preonce(start, end): 预热阶段 [0, minperiod-1)
 * - oncestart(start, end): 启动阶段 [minperiod-1, minperiod)
 * - once(start, end): 主计算阶�?[minperiod, buflen)
 */
...
public:
    ...
    /**
     * @brief 启动阶段计算
     * @param start 起始索引（通常�?minperiod-1�?
     * @param end 结束索引（通常�?minperiod�?
     * 
     * 第一个有效数据点
     * 默认调用 once()
     */
...
struct VectorizedStats {
    Size totalBars = 0;           // 总数据点�?
    Size computedBars = 0;        // 计算的数据点�?
    Size preonceBars = 0;         // preonce 阶段数据�?
    Size oncestartBars = 0;       // oncestart 阶段数据�?
    Size onceBars = 0;            // once 阶段数据�?
