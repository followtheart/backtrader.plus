/**
 * @file optimizer.hpp
 * @brief 策略参数优化系统 - 对应 Python �?optstrategy
 * 
 * 实现功能�?
 * - 参数网格搜索
 * - 多线程并行优�?
 ...
 * @brief ParamValue 比较�?
 * 
 * 自定义比较器，用于在 std::map 中使�?ParamValue 作为键�?
 * 处理 std::variant 类型，包�?nullptr_t�?
 */
...
struct OptResult {
    ...
    Value finalValue = 0;                       // 最终资产价�?
    Value pnl = 0;                              // 盈亏
    Value pnlPct = 0;                           // 盈亏百分�?
    Value sharpeRatio = NaN;                    // 夏普比率
    Value maxDrawdown = NaN;                    // 最大回�?
    Size totalTrades = 0;                       // 总交易数
    Size winningTrades = 0;                     // 盈利交易�?
    Value winRate = 0;                          // 胜率
 ...
 */
struct OptConfig {
    Size maxCpus = 0;               // 最�?CPU 数（0 = 自动检测）
    bool preload = true;            // 预加载数�?
    bool optDatas = true;           // 优化时共享数�?
